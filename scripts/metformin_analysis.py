...
def clopidogrel_negative_control_analysis(gp_scripts=None, true_clopidogrels=None, processed_ids=None, 
                                         thetas=None, sig_indices=None, covariate_dicts=None, Y=None, 
                                         mi_indices=None, cov=None):
    """
    Clopidogrel negative control analysis for CRC prevention
    ...
    Parameters: Same as metformin analysis
    Returns: Same structure for plotting compatibility
    """
    
    print("=== CLOPIDOGREL NEGATIVE CONTROL ANALYSIS (CRC PREVENTION) ===")
    ...
    # Step 1: Verify patient cohorts are properly defined
    print("\n1. Verifying patient cohort definitions:")
    cohort_ok = verify_patient_cohorts_metformin(gp_scripts, true_clopidogrels, processed_ids)
    if not cohort_ok:
        print("❌ Cohort definition failed - stopping analysis")
        return None
    ...
    treated_ok = verify_treated_patients_actually_have_metformin(treated_eids, true_clopidogrels)
    if not treated_ok:
        print("❌ Treated patient verification failed - stopping analysis")
        return None
    
    # Step 3: Define controls (patients not on clopidogrel)
    print("\n3. Defining controls:")
    if gp_scripts is not None:
        all_patients_with_prescriptions = set(gp_scripts['eid'].unique())
    else:
    ...
    # Find patients not on clopidogrel
    all_clopidogrel_eids = set(true_clopidogrels['eid'].unique())
    controls = [eid for eid in all_patients_with_prescriptions 
               if eid not in all_clopidogrel_eids]
    
    print(f"   Found {len(controls)} control patients (not on clopidogrel)")
    
    # VERIFY: Check that controls don't have clopidogrel
    control_ok = verify_controls_are_clean_metformin(controls, true_clopidogrels)
    if not control_ok:
        print("❌ Control verification failed - stopping analysis")
        return None
    
    # Use controls for matching
    control_eids_for_matching = controls[:len(treated_eids)*2]  # 2:1 ratio
    control_t0s = []
    valid_control_eids = []
