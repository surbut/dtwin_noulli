...
def comprehensive_treatment_analysis(signature_loadings, processed_ids, 
    ...
    key_covariates = ['tchol', 'hdl', 'SBP', 'pce_goff', 'sex', 
                     'dm2_prev', 'antihtnbase', 'dm1_prev']
    
    balance_stats = {}
    
    # First, assess age balance using index ages
    treated_index_ages = []
    control_index_ages = []
    
    # Get index ages for treated patients (treatment age)
    for eid in matched_treated_eids:
        age_at_enroll = covariate_dicts['age_at_enroll'].get(int(eid))
        if age_at_enroll is not None and not np.isnan(age_at_enroll):
            # Find treatment time for this patient
            treatment_time = None
            for i, treated_eid in enumerate(learner.treatment_patterns['treated_patients']):
                if treated_eid == eid:
                    treatment_time = learner.treatment_patterns['treatment_times'][i]
                    break
            if treatment_time is not None:
                treatment_age = age_at_enroll + (treatment_time / 12)
                treated_index_ages.append(treatment_age)
    
    # Get index ages for control patients (enrollment age)
    for eid in matched_control_eids:
        age_at_enroll = covariate_dicts['age_at_enroll'].get(int(eid))
        if age_at_enroll is not None and not np.isnan(age_at_enroll):
            control_index_ages.append(age_at_enroll)
    
    # Calculate SMD for index ages
    if len(treated_index_ages) > 0 and len(control_index_ages) > 0:
        ...
        smd = compute_smd(treated_index_ages, control_index_ages)
        balance_stats['index_age'] = {
            'treated_mean': np.mean(treated_index_ages),
            'control_mean': np.mean(control_index_ages),
            'treated_std': np.std(treated_index_ages),
            'control_std': np.std(control_index_ages),
            'smd': smd,
            'n_treated': len(treated_index_ages),
            'n_control': len(control_index_ages)
        }
    
    # Then assess other covariates
    ...
    if outcomes is not None:
        ...
        # Use the matched treated indices (not the original ones)
        # matched_treated_indices are the global indices of the matched treated patients
        
        # Subset outcomes to only the matched treated patients
        matched_treated_outcomes_full = outcomes_np[matched_treated_indices]
    ...
    # Step 7: Compare different matching approaches
    print("\n7. Comparing different matching approaches...")
    
    if outcomes is not None and event_indices is not None:
        comparison_results = compare_matching_approaches(
            signature_loadings, processed_ids, statin_prescriptions,
            covariates, covariate_dicts, outcomes, event_indices,
            matched_treated_indices, matched_control_indices
        )
    else:
        print("   Skipping comparison (no outcomes provided)")
        comparison_results = None
    
    # Step 8: Trial reproduction validation
    ...
    # Step 9: Visualization with matched controls
...
def compare_matching_approaches(signature_loadings, processed_ids, statin_prescriptions, 
                               covariates, covariate_dicts, outcomes, event_indices,
                               matched_treated_indices, matched_control_indices):
    """
    Compare different matching approaches:
    1. No matching (all treated vs all controls) - minimal exclusions to show confounding
    2. Clinical covariates only (no signatures)
    3. Signatures + covariates (current approach)
    """
    
    print("\n=== COMPARING MATCHING APPROACHES ===")
    
    # Convert outcomes to numpy if needed
    if hasattr(outcomes, 'detach'):
        outcomes_np = outcomes.detach().cpu().numpy()
    else:
        outcomes_np = outcomes
    
    # Get all treated and control patients
    learner = ObservationalTreatmentPatternLearner(
        signature_loadings, processed_ids, statin_prescriptions, covariates
    )
    all_treated_eids = learner.treatment_patterns['treated_patients']
    all_treated_times = learner.treatment_patterns['treatment_times']
    all_control_eids = learner.treatment_patterns['never_treated']
    
    # Filter to patients with prescription data
    if hasattr(learner, 'gp_scripts') and learner.gp_scripts is not None:
        all_patients_with_prescriptions = set(learner.gp_scripts['eid'].unique())
    else:
        all_patients_with_prescriptions = set(statin_prescriptions['eid'].unique())
    
    treated_patients_set = set(all_treated_eids)
    valid_control_eids = [eid for eid in all_patients_with_prescriptions 
                         if eid not in treated_patients_set and eid in processed_ids]
    
    # 1. NO MATCHING: Minimal exclusions (only basic missing data)
    print("1. No matching (all treated vs all controls)...")
    no_matching_treated_indices = []
    no_matching_control_indices = []
    
    # Minimal exclusions for treated patients (no matching)
    for i, eid in enumerate(all_treated_eids):
        if eid in processed_ids:
            # Only exclude if completely missing enrollment age
            age_at_enroll = covariate_dicts['age_at_enroll'].get(int(eid))
            if age_at_enroll is None or np.isnan(age_at_enroll):
                continue
            
            no_matching_treated_indices.append(np.where(processed_ids == eid)[0][0])
    
    # Minimal exclusions for control patients (no matching)
    for eid in valid_control_eids:  # Take ALL controls for no matching (no limit)
        if eid in processed_ids:
            # Only exclude if completely missing enrollment age
            age_at_enroll = covariate_dicts['age_at_enroll'].get(int(eid))
            if age_at_enroll is None or np.isnan(age_at_enroll):
            ...
            no_matching_control_indices.append(np.where(processed_ids == eid)[0][0])
    
    print(f"   No matching: Found {len(no_matching_treated_indices)} treated, {len(no_matching_control_indices)} controls")
    print(f"   Total patients with prescriptions: {len(all_patients_with_prescriptions)}")
    print(f"   Treated patients: {len(treated_patients_set)}")
    print(f"   Valid control eids (in processed_ids): {len(valid_control_eids)}")
    print(f"   No matching treated indices: {len(no_matching_treated_indices)}")
    
    # 2. CLINICAL MATCHING: Apply same exclusions as main analysis
    print("2. Clinical covariates only (no signatures)...")
    clinical_treated_indices = []
    clinical_control_indices = []
    
    # Apply same exclusions as main analysis for clinical matching
    for i, eid in enumerate(all_treated_eids):
        if eid in processed_ids:
            # Apply same exclusion logic as main analysis
            age_at_enroll = covariate_dicts['age_at_enroll'].get(int(eid), 57)
            treatment_age = age_at_enroll + (all_treated_times[i] / 12)
            ...
            antihtn = covariate_dicts['antihtnbase'].get(int(eid))
            dm1 = covariate_dicts['dm1_prev'].get(int(eid))
            if any(v is None or np.isnan(v) for v in [dm2, antihtn, dm1]):
                continue
            
            # Check missing disease status
            dm_any = covariate_dicts.get('Dm_Any', {}).get(int(eid))
            ht_any = covariate_dicts.get('Ht_Any', {}).get(int(eid))
            hyperlip_any = covariate_dicts.get('HyperLip_Any', {}).get(int(eid))
            if any(v is None or np.isnan(v) for v in [dm_any, ht_any, hyperlip_any]):
                continue
            
            clinical_treated_indices.append(np.where(processed_ids == eid)[0][0])
    
    # Apply same exclusions for clinical matching controls
    for eid in valid_control_eids[:len(clinical_treated_indices)*2]:
        if eid in processed_ids:
            age_at_enroll = covariate_dicts['age_at_enroll'].get(int(eid))
            if age_at_enroll is None or np.isnan(age_at_enroll):
                continue
            
            # Check CAD exclusion (enrollment date for controls)
            cad_any = covariate_dicts.get('Cad_Any', {}).get(int(eid), 0)
            cad_censor_age = covariate_dicts.get('Cad_censor_age', {}).get(int(eid))
            if cad_any == 2 and cad_censor_age is not None and not np.isnan(cad_censor_age):
                if cad_censor_age < age_at_enroll:
                    continue
            ...
            clinical_control_indices.append(np.where(processed_ids == eid)[0][0])
    
    results = {}
    
    # Approach 1: No matching (all treated vs all controls)
    print("\n1. No matching (all treated vs all controls)...")
    hr_no_matching = estimate_hr_for_groups(
        no_matching_treated_indices, no_matching_control_indices, outcomes_np, event_indices, 
        processed_ids, covariate_dicts, "No matching",
        treated_eids=all_treated_eids, treated_times=all_treated_times
    )
    results['no_matching'] = hr_no_matching
    
    # Approach 2: Clinical covariates only (no signatures)
    print("\n2. Clinical covariates only (no signatures)...")
    hr_clinical_only = estimate_hr_clinical_matching(
        all_treated_eids, valid_control_eids, processed_ids, signature_loadings,
        covariate_dicts, outcomes_np, event_indices, statin_prescriptions
    )
    results['clinical_only'] = hr_clinical_only
    
    # Approach 3: Current approach (signatures + covariates) - already done
    print("\n3. Current approach (signatures + covariates)...")
    hr_current = estimate_hr_for_groups(
        matched_treated_indices, matched_control_indices, outcomes_np, event_indices,
        processed_ids, covariate_dicts, "Current approach",
        treated_eids=all_treated_eids, treated_times=all_treated_times
    )
    results['current'] = hr_current
    
    # Print comparison summary
    print("\n=== MATCHING APPROACH COMPARISON ===")
    print(f"{'Approach':<25} {'HR':<8} {'95% CI':<15} {'P-value':<10} {'Events':<8}")
    print("-" * 70)
    
    for approach, result in results.items():
        if result is not None:
            hr = result['hazard_ratio']
            ci_lower = result['hr_ci_lower']
            ci_upper = result['hr_ci_upper']
            p_value = result['p_value']
            events = result['total_events']
            
            print(f"{approach:<25} {hr:<8.3f} {ci_lower:.3f}-{ci_upper:.3f} {p_value:<10.4f} {events:<8}")
        else:
            print(f"{approach:<25} {'Failed':<8} {'-':<15} {'-':<10} {'-':<8}")
    
    return results

def estimate_hr_for_groups(treated_indices, control_indices, outcomes, event_indices,
                          processed_ids, covariate_dicts, approach_name, 
                          treated_eids=None, treated_times=None):
    """
    Estimate HR for given treated and control groups
    """
    try:
        # Extract outcomes and follow-up times
        treated_outcomes = []
        control_outcomes = []
        follow_up_times = []
        ...
        for treated_idx in treated_indices:
            ...
            # Find treatment time
            treatment_time = None
            if treated_eids is not None and treated_times is not None:
                for i, eid in enumerate(treated_eids):
                    if eid == treated_eid:
                        treatment_time = treated_times[i]
                        break
            
            if treatment_time is not None and treated_idx < outcomes.shape[0]:
                # Check for events after treatment
                post_treatment_outcomes = outcomes[treated_idx, event_indices, int(treatment_time):]
                post_treatment_outcomes = np.any(post_treatment_outcomes > 0, axis=0)
...
def estimate_hr_clinical_matching(treated_eids, control_eids, processed_ids, signature_loadings,
                                 covariate_dicts, outcomes, event_indices, statin_prescriptions):
    """
    Estimate HR using matching on clinical covariates only (no signatures)
    """
    try:
        # Build features using only clinical covariates (no signatures)
        clinical_features_treated = []
        clinical_features_control = []
        treated_indices_clinical = []
        control_indices_clinical = []
        
        # Get treatment times
        learner = ObservationalTreatmentPatternLearner(
            signature_loadings, processed_ids, statin_prescriptions, None
        )
        treated_times = learner.treatment_patterns['treatment_times']
        
        # Build clinical features for treated patients
        for i, (eid, t0) in enumerate(zip(treated_eids, treated_times)):
            try:
                idx = np.where(processed_ids == int(eid))[0][0]
                
                # Apply same exclusion logic as main analysis
                age_at_enroll = covariate_dicts['age_at_enroll'].get(int(eid), 57)
                treatment_age = age_at_enroll + (t0 / 12)  # Convert months to years
                
                # Check CAD exclusion (incident user logic)
                cad_any = covariate_dicts.get('Cad_Any', {}).get(int(eid), 0)
                cad_censor_age = covariate_dicts.get('Cad_censor_age', {}).get(int(eid))
                if cad_any == 2 and cad_censor_age is not None and not np.isnan(cad_censor_age):
                    if cad_censor_age < treatment_age:
                ...
                # Check for missing disease status
                dm_any = covariate_dicts.get('Dm_Any', {}).get(int(eid))
                ht_any = covariate_dicts.get('Ht_Any', {}).get(int(eid))
                hyperlip_any = covariate_dicts.get('HyperLip_Any', {}).get(int(eid))
                ...
                hdl = covariate_dicts.get('hdl', {}).get(int(eid), 50)
                tchol = covariate_dicts.get('tchol', {}).get(int(eid), 200)
                sbp = covariate_dicts.get('SBP', {}).get(int(eid), 140)
                pce_goff = covariate_dicts.get('pce_goff', {}).get(int(eid), 0.09)
                smoke = encode_smoking(covariate_dicts.get('smoke', {}).get(int(eid), None))
                
                # Extract clinical variables
                age = treatment_age  # Age at treatment for treated patients
                sex = covariate_dicts['sex'].get(int(eid), 0)
                ldl_prs = covariate_dicts.get('ldl_prs', {}).get(int(eid), 0)
                hdl = covariate_dicts.get('hdl', {}).get(int(eid), 50)
                tchol = covariate_dicts.get('tchol', {}).get(int(eid), 200)
                ...
                # Create clinical feature vector (no signatures)
                clinical_vector = [age, sex, dm2, antihtn, dm1, ldl_prs, tchol, hdl, sbp, pce_goff] + smoke
                
                if not np.any(np.isnan(clinical_vector)):
                    clinical_features_treated.append(clinical_vector)
                    treated_indices_clinical.append(idx)
                    
            except:
                continue
        
        # Build clinical features for control patients
        for eid in control_eids[:len(clinical_features_treated)*2]:
            try:
                idx = np.where(processed_ids == int(eid))[0][0]
                age_at_enroll = covariate_dicts['age_at_enroll'].get(int(eid))
                
                if age_at_enroll is not None and not np.isnan(age_at_enroll):
                    # Same clinical feature extraction as above
                    age = age_at_enroll  # Age at enrollment for control patients
                    sex = covariate_dicts['sex'].get(int(eid), 0)
                    dm2 = covariate_dicts['dm2_prev'].get(int(eid))
                    antihtn = covariate_dicts['antihtnbase'].get(int(eid))
                    dm1 = covariate_dicts['dm1_prev'].get(int(eid))
                    ...
                    if not np.any(np.isnan(clinical_vector)):
                        clinical_features_control.append(clinical_vector)
                        control_indices_clinical.append(idx)
                        
            except:
                continue
        
        if len(clinical_features_treated) < 100 or len(clinical_features_control) < 100:
            print("   Clinical matching: Insufficient data")
            return None
        
        # Perform matching on clinical features only
        clinical_features_treated = np.array(clinical_features_treated)
        clinical_features_control = np.array(clinical_features_control)
        
        # Standardize and match
        scaler = StandardScaler()
        treated_std = scaler.fit_transform(clinical_features_treated)
        control_std = scaler.transform(clinical_features_control)
        
        nn = NearestNeighbors(n_neighbors=1, algorithm='auto').fit(control_std)
        distances, indices = nn.kneighbors(treated_std)
        
        matched_control_indices_clinical = [control_indices_clinical[i] for i in indices.flatten()]
        matched_treated_indices_clinical = treated_indices_clinical
        
        print(f"   Clinical matching: Matched {len(matched_treated_indices_clinical)} pairs")
        
        # Estimate HR for clinical matching
        return estimate_hr_for_groups(
            matched_treated_indices_clinical, matched_control_indices_clinical,
            outcomes, event_indices, processed_ids, covariate_dicts, "Clinical matching",
            treated_eids=treated_eids, treated_times=treated_times
        )
        
    except Exception as e:
        print(f"   Clinical matching: Error - {e}")
        return None
