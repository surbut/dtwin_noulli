...
def perform_matching(treated_features, control_features, treated_eids, control_eids, 
                    treated_indices, control_indices, method='nearest'):
    """
    Perform matching between treated and control patients
    
    Parameters:
    - treated_features: Feature matrix for treated patients
    - control_features: Feature matrix for control patients  
    - treated_eids: List of treated patient IDs
    - control_eids: List of control patient IDs
    - treated_indices: Indices of treated patients in thetas
    - control_indices: Indices of control patients in thetas
    - method: Matching method ('nearest' or 'greedy')
    
    Returns:
    - matched_treated: Matched treated patient indices
    - matched_controls: Matched control patient indices
    - matched_pairs: List of (treated_idx, control_idx) pairs
    """
    if method == 'nearest':
        ...
        for i, (treated_idx, control_idx) in enumerate(zip(range(len(treated_features)), indices.flatten())):
            matched_pairs.append((treated_idx, control_idx))
            matched_treated.append(treated_indices[treated_idx])
            matched_controls.append(control_indices[control_idx])
            
    elif method == 'greedy':
        # Greedy matching (1:1 without replacement)
        scaler = StandardScaler()
        treated_scaled = scaler.fit_transform(treated_features)
        control_scaled = scaler.transform(control_features)
        ...
        for i, treated_feat in enumerate(treated_scaled):
            ...
            for j, control_feat in enumerate(control_scaled):
                if j in used_controls:
                    continue
                    
                distance = np.linalg.norm(treated_feat - control_feat)
            ...
            if best_match is not None:
                ...
                matched_controls.append(control_indices[best_match])
                used_controls.add(best_match)
    
    print(f"Matched {len(matched_pairs)} pairs using {method} method")
    return matched_treated, matched_controls, matched_pairs

def calculate_hazard_ratio(treated_outcomes, control_outcomes, treated_times, control_times):
    """
    Calculate hazard ratio using Cox proportional hazards
    
    Parameters:
    - treated_outcomes: Binary outcomes for treated patients
    - control_outcomes: Binary outcomes for control patients
    - treated_times: Follow-up times for treated patients
    - control_times: Follow-up times for control patients
    
    Returns:
    - hazard_ratio: Point estimate of HR
    ...
    try:
        ...
        data = []
        for outcome, time in zip(treated_outcomes, treated_times):
            data.append({'event': outcome, 'time': time, 'group': 'treated'})
        for outcome, time in zip(control_outcomes, control_times):
            data.append({'event': outcome, 'time': time, 'group': 'control'})
            
        df = pd.DataFrame(data)
        
        # Fit Cox model
        cph = CoxPHFitter()
        cph.fit(df, duration_col='time', event_col='event', covariates=['group'])
        
        # Extract results
        hazard_ratio = np.exp(cph.params_['group'])
        ci_lower = np.exp(cph.confidence_intervals_.loc['group', 'lower 0.95'])
        ci_upper = np.exp(cph.confidence_intervals_.loc['group', 'upper 0.95'])
        p_value = cph.p_values_['group']
        
        return hazard_ratio, ci_lower, ci_upper, p_value
        
    except ImportError:
        print("Warning: lifelines not available, cannot calculate HR")
...
def simple_treatment_analysis(gp_scripts, true_statins, processed_ids, thetas, sig_indices, 
    ...
    if cad_exclusion_failed > 0:
        ...
        print("   ✅ PASSED: All patients properly excluded for CAD before index date")
